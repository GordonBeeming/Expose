...
namespace Expose
{
  public static class Settings
  {
    ...
    private const string CloudflareCName = "{A1ABE86B-DD46-4D92-9DC2-883839FBDE90}";
    ...
    internal const string SetCloudflareCNameContentCmd = @"[set.cf.config.cname] ""<cname>"" #### this option means you don't need to keep the wildcard entry in cloudflare###";
    internal static Task SetCloudflareCNameContent(string[] args)
    {
      var value = args[1];
      if (string.IsNullOrEmpty(value))
      {
        Output.WriteError($"CNAME should not be empty. Value provided is '{value}'.");
      }
      value = value.ToLowerInvariant();
      return SetSetting("Cloudflare CNAME", CloudflareCName, value);
    }

    internal static Task<string> GetCloudflareCNameContent()
    {
      return GetSetting(CloudflareCName);
    }
